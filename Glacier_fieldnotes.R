...
words <- comments2 %>% 
  ...
  mutate(arthropod = grepl("spider|bug|insect", ignore.case = TRUE,SPECIES.COMMENTS)) %>% 
  mutate(pollen = grepl("pollen", ignore.case = TRUE,SPECIES.COMMENTS)) %>% 
  mutate(seed = grepl("seed", ignore.case = TRUE,SPECIES.COMMENTS)) %>% 
...
aggregate(gl_sf$arthropod, by=list(gl_sf$COMMON.NAME), FUN=sum) # count arthropod observations
aggregate(gl_sf$pollen, by=list(gl_sf$COMMON.NAME), FUN=sum) # count pollen observations
aggregate(gl_sf$seed, by=list(gl_sf$COMMON.NAME), FUN=sum) # count seed observations
aggregate(gl_sf$worm, by=list(gl_sf$COMMON.NAME), FUN=sum) # count worm observations
...
aggregate(words_filter$arthropod, by=list(words_filter$COMMON.NAME), FUN=sum) # count arthropod observations
aggregate(words_filter$pollen, by=list(words_filter$COMMON.NAME), FUN=sum) # count pollen observations
aggregate(words_filter$seed, by=list(words_filter$COMMON.NAME), FUN=sum) # count seed observations
aggregate(words_filter$worm, by=list(words_filter$COMMON.NAME), FUN=sum) # count worm observations
